...
class ProfilePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<_SettingsSubMenuItemData> settingsListData = [
            ...
            BlocBuilder<MenuBloc, MenuState>(
              builder: (BuildContext context, MenuState state) {
                ...
                if (state is InitialMenuState) {
                  list.add(_ProfileAvatarStates.with2Letters(twoLetters: state.twoLetters.value));
                  list.add(_NameAndEditButtonStates.noName(onTapEdit: onTapEdit));
                  list.add(TouristIdState.noId());
                }

                if (state is UpdatedMenuState) {
                  if (state.image.isPresent == false && state.twoLetters.isPresent == true) {
                    list.add(_ProfileAvatarStates.with2Letters(twoLetters: state.twoLetters.value));
                  } else if (state.image.isPresent == true) {
                    list.add(_ProfileAvatarStates.withPicture(url: state.image.value));
                  }
...
class _ProfileAvatarStates extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (result == null) throw Exception('_AvatarProfileWithStates must select one of the states');
    return Row(
      mainAxisAlignment: MainAxisAlignment.start,
      crossAxisAlignment: CrossAxisAlignment.center,
      children: <Widget>[
            ...
            if (hasNotification)
              Container(
                ...
                decoration: BoxDecoration(
                    borderRadius: BorderRadius.circular(4), color: AppColors.primaryRed),
                height: 8,
                width: 8,
              ),
            Spacer(),
...
class _ProfileAvatarWithPicture extends StatelessWidget {
  ...
  const _ProfileAvatarWithPicture({Key key, @required this.url}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return _AvatarProfileBase(
      child: CachedNetworkImage(
        imageUrl: url,
        placeholder: (context, str) =>
            CircleAvatar(radius: 30, child: Center(child: CircularProgressIndicator())),
        imageBuilder: (BuildContext context, ImageProvider imageProvider) {
          return CircleAvatar(
            radius: 30,
            backgroundImage: imageProvider,
